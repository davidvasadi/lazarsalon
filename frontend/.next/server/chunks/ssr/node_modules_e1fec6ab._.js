...
"[project]/node_modules/@strapi/client/dist/bundle.mjs [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class r {
    _options;
    constructor(t){
        this._options = t, this.preflightValidation();
    }
}
class n extends Error {
    constructor(t = void 0, e = "An error occurred in the Strapi client. Please check the logs for more information."){
        super(e), this.cause = t;
    }
}
class o extends n {
    constructor(t = void 0, e = "Some of the provided values are not valid."){
        super(t, e);
    }
}
class i extends n {
    constructor(t = void 0, e = "Could not initialize the Strapi Client"){
        super(t, e);
    }
}
...
class s extends a {
    constructor(t){
        super(`Could not parse invalid URL: "${t}"`);
    }
}
class u extends Error {
    ...
    request;
    constructor(t, e){
        const r = `${t.status?.toString() ?? ""} ${t.statusText ?? ""}`.trim();
        super(`Request failed with ${r ? `status code ${r}` : "an unknown error"}: ${e.method} ${e.url}`), this.response = t, this.request = e;
    }
}
class c extends u {
...
const h = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:auth:provider:api-token");
class g extends r {
    ...
    preflightValidation() {
        if (h("validating provider configuration"), "string" != typeof this._token || 0 === this._token.trim().length) throw h("invalid api token provided: %o (%o)", this._token, typeof this._token), new o(`A valid API token is required when using the api-token auth strategy. Got "${this._token}"`);
        h("provider configuration validated successfully");
    }
    authenticate() {
        return h("no authentication step is required for the %o auth strategy, skipping", this.name), Promise.resolve();
    }
    get headers() {
        return {
...
const m = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:auth:provider:users-permissions"), b = "/auth/local";
class w extends r {
    ...
    preflightValidation() {
        if (m("validating provider configuration"), void 0 === this._options || null === this._options || "object" != typeof this._options) throw m("invalid options provided: %s (%s)", this._options, typeof this._options), new o("Missing valid options for initializing the Users & Permissions auth provider.");
        const { identifier: t, password: e } = this._options;
        if ("string" != typeof t) throw m("invalid identifier provided: %s (%s)", t, typeof t), new o(`The "identifier" option must be a string, but got "${typeof t}"`);
        if ("string" != typeof e) throw m("invalid password provided: %s (%s)", e, typeof e), new o(`The "password" option must be a string, but got "${typeof e}"`);
        m("provider configuration validated successfully");
    }
    get headers() {
    ...
    async authenticate(t) {
        const { identifier: e, password: r } = this._credentials;
        m("trying to authenticate with %o as %o at %o ", this.name, e, b);
        const n = await t.post(b, JSON.stringify({
            identifier: e,
            password: r
        }), {
            headers: {
        ...
        const o = await n.json(), i = o.jwt.slice(0, 5) + "..." + o.jwt.slice(-5);
        m("authentication successful for %o (%o)", e, i), this._token = o.jwt;
    }
}
const v = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:auth:factory");
class _ {
    _registry = new Map;
    create(t, e) {
        const r = this._registry.get(t);
        if (!r) throw v("the %o auth strategy is not registered, skipping", t), new n(`Auth strategy "${t}" is not supported.`);
        const o = r(e);
        return v("successfully instantiated a new %o provider", t), o;
    }
    register(t, e) {
        return this._registry.set(t, e), v("registered a new auth strategy: %o", t), this;
    }
}
const S = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:auth:manager");
class A {
    _authProviderFactory;
    _authProvider;
    _isAuthenticated = !1;
    constructor(t = new _){
        S("initializing a new auth manager"), this._authProviderFactory = t, this.registerDefaultProviders();
    }
    get strategy() {
    ...
    handleUnauthorizedError() {
        S("unauthorized error encountered, resetting authentication status"), this._isAuthenticated = !1;
    }
    setStrategy(t, e) {
        S("setting strategy to %o", t), this._authProvider = this._authProviderFactory.create(t, e);
    }
    async authenticate(t) {
        if (void 0 === this._authProvider) return S("no auth provider is set, skipping authentication"), void (this._isAuthenticated = !1);
        try {
            S("trying to authenticate with %s", this._authProvider.name);
            const e = t.create(void 0, !1);
            await this._authProvider.authenticate(e), this._isAuthenticated = !0, S("authentication successful");
        } catch (t) {
            S(`authentication failed: ${t}`), this._isAuthenticated = !1;
        }
    }
    authenticateRequest(t) {
        if (!this._authProvider) return;
        const { headers: e } = t;
        if (!(e instanceof Headers)) throw new Error(`Invalid request headers, headers must be an instance of Headers but found "${typeof e}"`);
        for (const [t, r] of Object.entries(this._authProvider.headers))e.set(t, r);
    }
    registerDefaultProviders() {
        S("registering default authentication providers"), this._authProviderFactory.register(g.identifier, (t)=>new g(t)).register(w.identifier, (t)=>new w(t)), S("default authentication providers registered successfully");
    }
}
var E, O, P, j, R, T, x, I, D, U, N, F, k, L, C, M, B, $, q, H, W, z, V, Q, K, G, J, Z, Y, X, tt, et, rt, nt, ot, it, at, st, ut, ct, lt, pt, ft, yt, dt, ht, gt, mt, bt, wt, vt, _t, St, At, Et, Ot, Pt, jt, Rt, Tt, xt, It, Dt, Ut, Nt, Ft, kt, Lt, Ct, Mt, Bt, $t, qt, Ht, Wt, zt, Vt, Qt, Kt, Gt, Jt = "undefined" != typeof globalThis ? globalThis : ("TURBOPACK compile-time falsy", 0) ? "TURBOPACK unreachable" : ("TURBOPACK compile-time truthy", 1) ? /*TURBOPACK member replacement*/ __turbopack_context__.g : "TURBOPACK unreachable";
function Zt(t) {
    return t && t.__esModule && Object.prototype.hasOwnProperty.call(t, "default") ? t.default : t;
}
function Yt() {
    return O ? E : (O = 1, E = TypeError);
}
function Xt() {
    return j ? P : (j = 1, P = __TURBOPACK__imported__module__$5b$externals$5d2f$util__$5b$external$5d$__$28$util$2c$__cjs$29$__["default"].inspect);
}
function te() {
    if (T) return R;
    T = 1;
    ...
    function U(t, e) {
        ...
        if ("number" == typeof t) {
            var n = t < 0 ? -A(-t) : A(t);
            if (n !== t) {
                var o = String(n), i = h.call(e, o.length + 1);
                return g.call(o, r, "$&_") + "." + g.call(g.call(i, /([0-9]{3})/g, "$&_"), /_$/, "");
            }
        }
        return g.call(e, r, "$&_");
    }
    var N = Xt(), F = N.custom, k = H(F) ? F : null, L = {
        __proto__: null,
        double: '"',
        single: "'"
    }, C = {
    ...
    R = function t(e, o, i, f) {
        var m = o || {};
        if (z(m, "quoteStyle") && !z(L, m.quoteStyle)) throw new TypeError('option "quoteStyle" must be "single" or "double"');
        if (z(m, "maxStringLength") && ("number" == typeof m.maxStringLength ? m.maxStringLength < 0 && m.maxStringLength !== 1 / 0 : null !== m.maxStringLength)) throw new TypeError('option "maxStringLength", if provided, must be a positive integer, Infinity, or `null`');
        var w = !z(m, "customInspect") || m.customInspect;
        if ("boolean" != typeof w && "symbol" !== w) throw new TypeError("option \"customInspect\", if provided, must be `true`, `false`, or `'symbol'`");
        if (z(m, "indent") && null !== m.indent && "\t" !== m.indent && !(parseInt(m.indent, 10) === m.indent && m.indent > 0)) throw new TypeError('option "indent" must be "\\t", an integer > 0, or `null`');
        if (z(m, "numericSeparator") && "boolean" != typeof m.numericSeparator) throw new TypeError('option "numericSeparator", if provided, must be `true` or `false`');
        var A = m.numericSeparator;
        if (void 0 === e) return "undefined";
        if (null === e) return "null";
        if ("boolean" == typeof e) return e ? "true" : "false";
        if ("string" == typeof e) return K(e, m);
        if ("number" == typeof e) {
        ...
        var T = void 0 === m.depth ? 5 : m.depth;
        if (void 0 === i && (i = 0), i >= T && T > 0 && "object" == typeof e) return $(e) ? "[Array]" : "[Object]";
        var F = function(t, e) {
            var r;
            if ("\t" === t.indent) r = "\t";
            else {
        ...
        if (void 0 === f) f = [];
        else if (Q(f, e) >= 0) return "[Circular]";
        function C(e, r, n) {
            if (r && (f = S.call(f)).push(r), n) {
                var o = {
                    depth: m.depth
                };
                return z(m, "quoteStyle") && (o.quoteStyle = m.quoteStyle), t(e, o, i + 1, f);
            }
            return t(e, m, i + 1, f);
        }
        if ("function" == typeof e && !q(e)) {
            var W = function(t) {
                ...
                var e = d.call(y.call(t), /^function\s*([\w$]+)/);
                if (e) return e[1];
                return null;
            }(e), G = tt(e, C);
            return "[Function" + (W ? ": " + W : " (anonymous)") + "]" + (G.length > 0 ? " { " + _.call(G, ", ") + " }" : "");
        }
        if (H(e)) {
            var et = j ? g.call(String(e), /^(Symbol\(.*\))_[^)]*$/, "$1") : P.call(e);
        ...
        if (function(t) {
            ...
            for(var rt = "<" + b.call(String(e.nodeName)), nt = e.attributes || [], ot = 0; ot < nt.length; ot++)rt += " " + nt[ot].name + "=" + M(B(nt[ot].value), "double", m);
            return rt += ">", e.childNodes && e.childNodes.length && (rt += "..."), rt += "</" + b.call(String(e.nodeName)) + ">";
        }
        if ($(e)) {
        ...
        if (function(t) {
            ...
            var at = tt(e, C);
            return "cause" in Error.prototype || !("cause" in e) || I.call(e, "cause") ? 0 === at.length ? "[" + String(e) + "]" : "{ [" + String(e) + "] " + _.call(at, ", ") + " }" : "{ [" + String(e) + "] " + _.call(v.call("[cause]: " + C(e.cause), at), ", ") + " }";
        }
        if ("object" == typeof e && w) {
            if (k && "function" == typeof e[k] && N) return N(e, {
                depth: T - i
            });
        ...
        if (function(t) {
            ...
            var st = [];
            return n && n.call(e, function(t, r) {
                st.push(C(r, e, !0) + " => " + C(t, e));
            }), Y("Map", r.call(e), st, F);
        }
        if (function(t) {
        ...
        if (function(t) {
            ...
            return !1;
        }(e)) return J(C(E.call(e)));
        if (function(t) {
            return !("[object Boolean]" !== V(t) || x && "object" == typeof t && x in t);
        }(e)) return J(p.call(e));
        if (function(t) {
            return !("[object String]" !== V(t) || x && "object" == typeof t && x in t);
        }(e)) return J(C(String(e)));
        if ("TURBOPACK compile-time falsy", 0) //TURBOPACK unreachable
        ;
        if ("undefined" != typeof globalThis && e === globalThis || void 0 !== Jt && e === Jt) return "{ [object globalThis] }";
        if (!function(t) {
            return !("[object Date]" !== V(t) || x && "object" == typeof t && x in t);
        }(e) && !q(e)) {
            var ct = tt(e, C), lt = D ? D(e) === Object.prototype : e instanceof Object || e.constructor === Object, pt = e instanceof Object ? "" : "null prototype", ft = !lt && x && Object(e) === e && x in e ? h.call(V(e), 8, -1) : pt ? "Object" : "", yt = (lt || "function" != typeof e.constructor ? "" : e.constructor.name ? e.constructor.name + " " : "") + (ft || pt ? "[" + _.call(v.call([], ft || [], pt || []), ": ") + "] " : "");
            return 0 === ct.length ? yt + "{}" : F ? yt + "{" + X(ct, F) + "}" : yt + "{ " + _.call(ct, ", ") + " }";
        }
        return String(e);
    };
    ...
    function K(t, e) {
        if (t.length > e.maxStringLength) {
            ...
            return K(h.call(t, 0, e.maxStringLength), e) + n;
        }
        var o = C[e.quoteStyle || "single"];
        return o.lastIndex = 0, M(g.call(g.call(t, o, "\\$1"), /[\x00-\x1f]/g, G), "single", e);
    }
    function G(t) {
        ...
        return r ? "\\" + r : "\\x" + (e < 16 ? "0" : "") + m.call(e.toString(16));
    }
    function J(t) {
        return "Object(" + t + ")";
    }
    ...
    function tt(t, e) {
        ...
        for(var u in t)z(t, u) && (r && String(Number(u)) === u && u < t.length || j && i["$" + u] instanceof Symbol || (w.call(/[^\w$]/, u) ? n.push(e(u, t) + ": " + e(t[u], t)) : n.push(u + ": " + e(t[u], t))));
        if ("function" == typeof O) for(var c = 0; c < a.length; c++)I.call(t, a[c]) && n.push("[" + e(a[c]) + "]: " + e(t[a[c]], t));
        return n;
    }
    return R;
}
function ee() {
    if (I) return x;
    I = 1;
    var t = te(), e = Yt(), r = function(t, e, r) {
        for(var n, o = t; null != (n = o.next); o = n)if (n.key === e) return o.next = n.next, r || (n.next = t.next, t.next = n), n;
    };
    return x = function() {
        var n, o = {
            assert: function(r) {
            ...
            delete: function(t) {
                var e = n && n.next, o = function(t, e) {
                    if (t) return r(t, e, !0);
                }(n, t);
                return o && e && e === o && (n = void 0), !!o;
            },
            get: function(t) {
                return function(t, e) {
                    if (t) {
...
function re() {
    return U ? D : (U = 1, D = Object);
}
function ne() {
    return F ? N : (F = 1, N = Error);
}
function oe() {
    return L ? k : (L = 1, k = EvalError);
}
function ie() {
    return M ? C : (M = 1, C = RangeError);
}
function ae() {
    return $ ? B : ($ = 1, B = ReferenceError);
}
function se() {
    return H ? q : (H = 1, q = SyntaxError);
}
function ue() {
    return z ? W : (z = 1, W = URIError);
}
function ce() {
    return Q ? V : (Q = 1, V = Math.abs);
}
function le() {
    return G ? K : (G = 1, K = Math.floor);
}
function pe() {
    return Z ? J : (Z = 1, J = Math.max);
}
function fe() {
    return X ? Y : (X = 1, Y = Math.min);
}
function ye() {
    return et ? tt : (et = 1, tt = Math.pow);
}
function de() {
    return nt ? rt : (nt = 1, rt = Object.getOwnPropertyDescriptor);
}
function he() {
    if (it) return ot;
    it = 1;
...
function me() {
    if (pt) return lt;
    pt = 1;
    var t = "undefined" != typeof Symbol && Symbol, e = ct ? ut : (ct = 1, ut = function() {
...
function be() {
    ...
    return ft = function(n) {
        ...
        for(var i, a = function(t, e) {
            for(var r = [], n = e, o = 0; n < t.length; n += 1, o += 1)r[o] = t[n];
            return r;
        }(arguments, 1), s = e(0, o.length - a.length), u = [], c = 0; c < s; c++)u[c] = "$" + c;
...
function we() {
    ...
    ht = 1;
    var t = be();
    return dt = Function.prototype.bind || t;
}
function ve() {
    return mt ? gt : (mt = 1, gt = Function.prototype.call);
}
function _e() {
    return wt ? bt : (wt = 1, bt = Function.prototype.apply);
}
function Se() {
    if (At) return St;
    At = 1;
    var t = we(), e = _e(), r = ve(), n = _t ? vt : (_t = 1, vt = "undefined" != typeof Reflect && Reflect && Reflect.apply);
    return St = n || t.call(r, e);
}
function Ae() {
    if (Ot) return Et;
    Ot = 1;
    var t = we(), e = Yt(), r = ve(), n = Se();
    return Et = function(o) {
        if (o.length < 1 || "function" != typeof o[0]) throw new e("a function is required");
        return n(t, r, o);
    };
}
function Ee() {
    ...
    var n = !!t && r && r(Object.prototype, "__proto__"), o = Object, i = o.getPrototypeOf;
    return Pt = n && "function" == typeof n.get ? e([
        n.get
...
function Oe() {
    ...
    return Rt = r.call(t, e);
}
function Pe() {
    if (It) return xt;
    var t;
    It = 1;
    var e = re(), r = ne(), n = oe(), o = ie(), i = ae(), a = se(), s = Yt(), u = ue(), c = ce(), l = le(), p = pe(), f = fe(), y = ye(), d = Function, h = function(t) {
        try {
            return d('"use strict"; return (' + t + ").constructor;")();
        } catch (t) {}
    }, g = he(), m = ge(), b = function() {
        throw new s;
    }, w = g ? function() {
        try {
            return b;
    ...
    }() : b, v = me()(), _ = Ee(), S = "function" == typeof Reflect && Reflect.getPrototypeOf || e.getPrototypeOf || _, A = _e(), E = ve(), O = {}, P = "undefined" != typeof Uint8Array && S ? S(Uint8Array) : t, j = {
        __proto__: null,
        "%AggregateError%": "undefined" == typeof AggregateError ? t : AggregateError,
        "%Array%": Array,
        "%ArrayBuffer%": "undefined" == typeof ArrayBuffer ? t : ArrayBuffer,
        "%ArrayIteratorPrototype%": v && S ? S([][Symbol.iterator]()) : t,
        ...
        "%isFinite%": isFinite,
        "%isNaN%": isNaN,
        "%IteratorPrototype%": v && S ? S(S([][Symbol.iterator]())) : t,
        "%JSON%": "object" == typeof JSON ? JSON : t,
        "%Map%": "undefined" == typeof Map ? t : Map,
        "%MapIteratorPrototype%": "undefined" != typeof Map && v && S ? S((new Map)[Symbol.iterator]()) : t,
        "%Math%": Math,
        "%Number%": Number,
        "%Object%": e,
        ...
        "%SetIteratorPrototype%": "undefined" != typeof Set && v && S ? S((new Set)[Symbol.iterator]()) : t,
        "%SharedArrayBuffer%": "undefined" == typeof SharedArrayBuffer ? t : SharedArrayBuffer,
        "%String%": String,
        "%StringIteratorPrototype%": v && S ? S(""[Symbol.iterator]()) : t,
        "%Symbol%": v ? Symbol : t,
        "%SyntaxError%": a,
        "%ThrowTypeError%": w,
        "%TypedArray%": P,
    ...
    }, I = we(), D = Oe(), U = I.call(E, Array.prototype.concat), N = I.call(A, Array.prototype.splice), F = I.call(E, String.prototype.replace), k = I.call(E, String.prototype.slice), L = I.call(E, RegExp.prototype.exec), C = /[^%.[\]]+|\[(?:(-?\d+(?:\.\d+)?)|(["'])((?:(?!\2)[^\\]|\\.)*?)\2)\]|(?=(?:\.|\[\])(?:\.|\[\]|%$))/g, M = /\\(\\)?/g, B = function(t, e) {
        var r, n = t;
        if (D(x, n) && (n = "%" + (r = x[n])[0] + "%"), D(j, n)) {
            var o = j[n];
            if (o === O && (o = T(n)), void 0 === o && !e) throw new s("intrinsic " + t + " exists, but is not available. Please file an issue!");
            return {
                alias: r,
                name: n,
                value: o
    ...
    return xt = function(t, e) {
        ...
        if (null === L(/^%?[^%]*%?$/, t)) throw new a("`%` may not be present anywhere but at the beginning and end of the intrinsic name");
        var r = function(t) {
            var e = k(t, 0, 1), r = k(t, -1);
            if ("%" === e && "%" !== r) throw new a("invalid intrinsic syntax, expected closing `%`");
            if ("%" === r && "%" !== e) throw new a("invalid intrinsic syntax, expected opening `%`");
            var n = [];
            return F(t, C, function(t, e, r, o) {
                n[n.length] = r ? F(o, M, "$1") : e || t;
            }), n;
        }(t), n = r.length > 0 ? r[0] : "", o = B("%" + n + "%", e), i = o.name, u = o.value, c = !1, l = o.alias;
        l && (n = l[0], N(r, U([
            0,
            1
        ], l)));
        for(var p = 1, f = !0; p < r.length; p += 1){
            var y = r[p], d = k(y, 0, 1), h = k(y, -1);
            if (('"' === d || "'" === d || "`" === d || '"' === h || "'" === h || "`" === h) && d !== h) throw new a("property names with quotes must have matching quotes");
            if ("constructor" !== y && f || (c = !0), D(j, i = "%" + (n += "." + y) + "%")) u = j[i];
            else if (null != u) {
                if (!(y in u)) {
                ...
                if (g && p + 1 >= r.length) {
                    var m = g(u, y);
                    u = (f = !!m) && "get" in m && !("originalValue" in m.get) ? m.get : u[y];
                } else f = D(u, y), u = u[y];
                f && !c && (j[i] = u);
            }
        }
...
function je() {
    ...
    return Dt = function(n, o) {
        var i = t(n, !!o);
        return "function" == typeof i && r(n, ".prototype.") > -1 ? e([
            i
        ]) : i;
    };
}
function Re() {
    if (Ft) return Nt;
    Ft = 1;
    var t = Pe(), e = je(), r = te(), n = Yt(), o = t("%Map%", !0), i = e("Map.prototype.get", !0), a = e("Map.prototype.set", !0), s = e("Map.prototype.has", !0), u = e("Map.prototype.delete", !0), c = e("Map.prototype.size", !0);
...
function Te() {
    if (Mt) return Ct;
    Mt = 1;
    var t = Yt(), e = te(), r = ee(), n = Re(), o = function() {
        if (Lt) return kt;
        Lt = 1;
        var t = Pe(), e = je(), r = te(), n = Re(), o = Yt(), i = t("%WeakMap%", !0), a = e("WeakMap.prototype.get", !0), s = e("WeakMap.prototype.set", !0), u = e("WeakMap.prototype.has", !0), c = e("WeakMap.prototype.delete", !0);
        return kt = i ? function() {
            var t, e, l = {
                assert: function(t) {
                    if (!l.has(t)) throw new o("Side channel does not contain " + r(t));
                },
                delete: function(r) {
                    if (i && r && ("object" == typeof r || "function" == typeof r)) {
                        if (t) return c(t, r);
                    } else if (n && e) return e.delete(r);
                    return !1;
                },
                get: function(r) {
                    return i && r && ("object" == typeof r || "function" == typeof r) && t ? a(t, r) : e && e.get(r);
                },
                has: function(r) {
                    return i && r && ("object" == typeof r || "function" == typeof r) && t ? u(t, r) : !!e && e.has(r);
                },
                set: function(r, o) {
                    i && r && ("object" == typeof r || "function" == typeof r) ? (t || (t = new i), s(t, r, o)) : n && (e || (e = n()), e.set(r, o));
                }
            };
            return l;
        } : n;
...
function Ie() {
    if (Ht) return qt;
    Ht = 1;
    var t = xe(), e = Object.prototype.hasOwnProperty, r = Array.isArray, n = function() {
    ...
    return qt = {
        ...
        compact: function(t) {
            ...
            ], n = [], o = 0; o < e.length; ++o)for(var i = e[o], a = i.obj[i.prop], s = Object.keys(a), u = 0; u < s.length; ++u){
                var c = s[u], l = a[c];
        ...
        encode: function(e, r, o, a, s) {
            if (0 === e.length) return e;
            var u = e;
            if ("symbol" == typeof e ? u = Symbol.prototype.toString.call(e) : "string" != typeof e && (u = String(e)), "iso-8859-1" === o) return escape(u).replace(/%u[0-9a-f]{4}/gi, function(t) {
                return "%26%23" + parseInt(t.slice(2), 16) + "%3B";
            });
            for(var c = "", l = 0; l < u.length; l += i){
                for(var p = u.length >= i ? u.slice(l, l + i) : u, f = [], y = 0; y < p.length; ++y){
                    var d = p.charCodeAt(y);
                    45 === d || 46 === d || 95 === d || 126 === d || d >= 48 && d <= 57 || d >= 65 && d <= 90 || d >= 97 && d <= 122 || s === t.RFC1738 && (40 === d || 41 === d) ? f[f.length] = p.charAt(y) : d < 128 ? f[f.length] = n[d] : d < 2048 ? f[f.length] = n[192 | d >> 6] + n[128 | 63 & d] : d < 55296 || d >= 57344 ? f[f.length] = n[224 | d >> 12] + n[128 | d >> 6 & 63] + n[128 | 63 & d] : (y += 1, d = 65536 + ((1023 & d) << 10 | 1023 & p.charCodeAt(y)), f[f.length] = n[240 | d >> 18] + n[128 | d >> 12 & 63] + n[128 | d >> 6 & 63] + n[128 | 63 & d]);
                }
                c += f.join("");
            }
            return c;
        ...
        merge: function t(n, i, a) {
            ...
            if ("object" != typeof i && "function" != typeof i) {
                if (r(n)) n.push(i);
                    ...
                    (a && (a.plainObjects || a.allowPrototypes) || !e.call(Object.prototype, i)) && (n[i] = !0);
                }
                return n;
            }
            if (!n || "object" != typeof n) return [
            ...
            var s = n;
            return r(n) && !r(i) && (s = o(n, a)), r(n) && r(i) ? (i.forEach(function(r, o) {
                if (e.call(n, o)) {
                    var i = n[o];
                    i && "object" == typeof i && r && "object" == typeof r ? n[o] = t(i, r, a) : n.push(r);
                } else n[o] = r;
            }), n) : Object.keys(i).reduce(function(r, n) {
                var o = i[n];
                return e.call(r, n) ? r[n] = t(r[n], o, a) : r[n] = o, r;
            }, s);
        }
    }, qt;
}
function De() {
    if (zt) return Wt;
    zt = 1;
    var t = Te(), e = Ie(), r = xe(), n = Object.prototype.hasOwnProperty, o = {
        brackets: function(t) {
            return t + "[]";
        },
        comma: "comma",
    ...
    }, i = Array.isArray, a = Array.prototype.push, s = function(t, e) {
        a.apply(t, i(e) ? e : [
            e
        ]);
    }, u = Date.prototype.toISOString, c = r.default, l = {
        addQueryPrefix: !1,
        allowDots: !1,
        allowEmptyArrays: !1,
        arrayFormat: "indices",
    ...
    }, p = {}, f = function r(n, o, a, u, c, f, y, d, h, g, m, b, w, v, _, S, A, E) {
        for(var O, P = n, j = E, R = 0, T = !1; void 0 !== (j = j.get(p)) && !T;){
            var x = j.get(n);
            if (R += 1, void 0 !== x) {
                if (x === R) throw new RangeError("Cyclic object value");
                T = !0;
            }
            void 0 === j.get(p) && (R = 0);
        }
        if ("function" == typeof g ? P = g(o, P) : P instanceof Date ? P = w(P) : "comma" === a && i(P) && (P = e.maybeMap(P, function(t) {
            return t instanceof Date ? w(t) : t;
        })), null === P) {
            if (f) return h && !S ? h(o, l.encoder, A, "key", v) : o;
            P = "";
        }
        if ("string" == typeof (O = P) || "number" == typeof O || "boolean" == typeof O || "symbol" == typeof O || "bigint" == typeof O || e.isBuffer(P)) return h ? [
            _(S ? o : h(o, l.encoder, A, "key", v)) + "=" + _(h(P, l.encoder, A, "value", v))
        ] : [
            _(o) + "=" + _(String(P))
        ];
        var I, D = [];
        if (void 0 === P) return D;
        if ("comma" === a && i(P)) S && h && (P = e.maybeMap(P, h)), I = [
            {
        ...
        var N = d ? String(o).replace(/\./g, "%2E") : String(o), F = u && i(P) && 1 === P.length ? N + "[]" : N;
        if (c && i(P) && 0 === P.length) return F + "[]";
        for(var k = 0; k < I.length; ++k){
            var L = I[k], C = "object" == typeof L && L && void 0 !== L.value ? L.value : P[L];
            if (!y || null !== C) {
                var M = b && d ? String(L).replace(/\./g, "%2E") : String(L), B = i(P) ? "function" == typeof a ? a(F, M) : F : F + (b ? "." + M : "[" + M + "]");
                E.set(n, R);
                var $ = t();
                $.set(p, E), s(D, r(C, B, a, u, c, f, y, d, "comma" === a && S && i(P) ? null : h, g, m, b, w, v, _, S, A, $));
            }
        }
        return D;
    };
    return Wt = function(e, a) {
        var u, c = e, p = function(t) {
            if (!t) return l;
            ...
            var a = r.default;
            if (void 0 !== t.format) {
                if (!n.call(r.formatters, t.format)) throw new TypeError("Unknown format option provided.");
                a = t.format;
            }
            var s, u = r.formatters[a], c = l.filter;
            if (("function" == typeof t.filter || i(t.filter)) && (c = t.filter), s = t.arrayFormat in o ? t.arrayFormat : "indices" in t ? t.indices ? "indices" : "repeat" : l.arrayFormat, "commaRoundTrip" in t && "boolean" != typeof t.commaRoundTrip) throw new TypeError("`commaRoundTrip` must be a boolean, or absent");
            var p = void 0 === t.allowDots ? !0 === t.encodeDotInKeys || l.allowDots : !!t.allowDots;
            return {
                ...
                encodeDotInKeys: "boolean" == typeof t.encodeDotInKeys ? t.encodeDotInKeys : l.encodeDotInKeys,
        ...
        "function" == typeof p.filter ? c = (0, p.filter)("", c) : i(p.filter) && (u = p.filter);
        var y = [];
        if ("object" != typeof c || null === c) return "";
        var d = o[p.arrayFormat], h = "comma" === d && p.commaRoundTrip;
        u || (u = Object.keys(c)), p.sort && u.sort(p.sort);
        for(var g = t(), m = 0; m < u.length; ++m){
            var b = u[m], w = c[b];
            p.skipNulls && null === w || s(y, f(w, b, d, h, p.allowEmptyArrays, p.strictNullHandling, p.skipNulls, p.encodeDotInKeys, p.encode ? p.encoder : null, p.filter, p.sort, p.allowDots, p.serializeDate, p.format, p.formatter, p.encodeValuesOnly, p.charset, g));
        }
        var v = y.join(p.delimiter), _ = !0 === p.addQueryPrefix ? "?" : "";
        return p.charsetSentinel && ("iso-8859-1" === p.charset ? _ += "utf8=%26%2310003%3B&" : _ += "utf8=%E2%9C%93&"), v.length > 0 ? _ + v : "";
    }, Wt;
}
function Ue() {
    if (Qt) return Vt;
    Qt = 1;
    var t = Ie(), e = Object.prototype.hasOwnProperty, r = Array.isArray, n = {
        allowDots: !1,
        allowEmptyArrays: !1,
        allowPrototypes: !1,
        allowSparse: !1,
        ...
        if (r) {
            var s = o.allowDots ? r.replace(/\.([^.[]+)/g, "[$1]") : r, u = /(\[[^[\]]*])/g, c = o.depth > 0 && /(\[[^[\]]*])/.exec(s), l = c ? s.slice(0, c.index) : s, p = [];
            if (l) {
                if (!o.plainObjects && e.call(Object.prototype, l) && !o.allowPrototypes) return;
                p.push(l);
            }
            for(var f = 0; o.depth > 0 && null !== (c = u.exec(s)) && f < o.depth;){
                if (f += 1, !o.plainObjects && e.call(Object.prototype, c[1].slice(1, -1)) && !o.allowPrototypes) return;
                p.push(c[1]);
            }
            if (c) {
                if (!0 === o.strictDepth) throw new RangeError("Input depth exceeded depth option of " + o.depth + " and strictDepth is true");
                p.push("[" + s.slice(c.index) + "]");
            }
            return function(e, r, n, o) {
                var a = 0;
                if (e.length > 0 && "[]" === e[e.length - 1]) {
                    var s = e.slice(0, -1).join("");
                    a = Array.isArray(r) && r[s] ? r[s].length : 0;
                }
                for(var u = o ? r : i(r, n, a), c = e.length - 1; c >= 0; --c){
                    var l, p = e[c];
                    if ("[]" === p && n.parseArrays) l = n.allowEmptyArrays && ("" === u || n.strictNullHandling && null === u) ? [] : t.combine([], u);
                    else {
                        l = n.plainObjects ? {
                            __proto__: null
                        } : {};
                        var f = "[" === p.charAt(0) && "]" === p.charAt(p.length - 1) ? p.slice(1, -1) : p, y = n.decodeDotInKeys ? f.replace(/%2E/g, ".") : f, d = parseInt(y, 10);
                        n.parseArrays || "" !== y ? !isNaN(d) && p !== y && String(d) === y && d >= 0 && n.parseArrays && d <= n.arrayLimit ? (l = [])[d] = u : "__proto__" !== y && (l[y] = u) : l = {
                            0: u
                        };
                    }
                    u = l;
    ...
    return Vt = function(s, u) {
        var c = function(e) {
            ...
            var r = void 0 === e.charset ? n.charset : e.charset, o = void 0 === e.duplicates ? n.duplicates : e.duplicates;
            if ("combine" !== o && "first" !== o && "last" !== o) throw new TypeError("The duplicates option must be either combine, first, or last");
            return {
                allowDots: void 0 === e.allowDots ? !0 === e.decodeDotInKeys || n.allowDots : !!e.allowDots,
                allowEmptyArrays: "boolean" == typeof e.allowEmptyArrays ? !!e.allowEmptyArrays : n.allowEmptyArrays,
                allowPrototypes: "boolean" == typeof e.allowPrototypes ? e.allowPrototypes : n.allowPrototypes,
                allowSparse: "boolean" == typeof e.allowSparse ? e.allowSparse : n.allowSparse,
                arrayLimit: "number" == typeof e.arrayLimit ? e.arrayLimit : n.arrayLimit,
                charset: r,
                charsetSentinel: "boolean" == typeof e.charsetSentinel ? e.charsetSentinel : n.charsetSentinel,
                comma: "boolean" == typeof e.comma ? e.comma : n.comma,
                decodeDotInKeys: "boolean" == typeof e.decodeDotInKeys ? e.decodeDotInKeys : n.decodeDotInKeys,
                decoder: "function" == typeof e.decoder ? e.decoder : n.decoder,
                delimiter: "string" == typeof e.delimiter || t.isRegExp(e.delimiter) ? e.delimiter : n.delimiter,
                depth: "number" == typeof e.depth || !1 === e.depth ? +e.depth : n.depth,
                duplicates: o,
                ignoreQueryPrefix: !0 === e.ignoreQueryPrefix,
                interpretNumericEntities: "boolean" == typeof e.interpretNumericEntities ? e.interpretNumericEntities : n.interpretNumericEntities,
                parameterLimit: "number" == typeof e.parameterLimit ? e.parameterLimit : n.parameterLimit,
                parseArrays: !1 !== e.parseArrays,
                plainObjects: "boolean" == typeof e.plainObjects ? e.plainObjects : n.plainObjects,
                strictDepth: "boolean" == typeof e.strictDepth ? !!e.strictDepth : n.strictDepth,
                strictNullHandling: "boolean" == typeof e.strictNullHandling ? e.strictNullHandling : n.strictNullHandling,
                throwOnLimitExceeded: "boolean" == typeof e.throwOnLimitExceeded && e.throwOnLimitExceeded
            };
        }(u);
        if ("" === s || null == s) return c.plainObjects ? {
            __proto__: null
        } : {};
        for(var l = "string" == typeof s ? function(a, s) {
            var u = {
                __proto__: null
            }, c = s.ignoreQueryPrefix ? a.replace(/^\?/, "") : a;
            c = c.replace(/%5B/gi, "[").replace(/%5D/gi, "]");
            var l = s.parameterLimit === 1 / 0 ? void 0 : s.parameterLimit, p = c.split(s.delimiter, s.throwOnLimitExceeded ? l + 1 : l);
            if (s.throwOnLimitExceeded && p.length > l) throw new RangeError("Parameter limit exceeded. Only " + l + " parameter" + (1 === l ? "" : "s") + " allowed.");
            var f, y = -1, d = s.charset;
            if (s.charsetSentinel) for(f = 0; f < p.length; ++f)0 === p[f].indexOf("utf8=") && ("utf8=%E2%9C%93" === p[f] ? d = "utf-8" : "utf8=%26%2310003%3B" === p[f] && (d = "iso-8859-1"), y = f, f = p.length);
            for(f = 0; f < p.length; ++f)if (f !== y) {
                var h, g, m = p[f], b = m.indexOf("]="), w = -1 === b ? m.indexOf("=") : b + 1;
                -1 === w ? (h = s.decoder(m, n.decoder, d, "key"), g = s.strictNullHandling ? null : "") : (h = s.decoder(m.slice(0, w), n.decoder, d, "key"), g = t.maybeMap(i(m.slice(w + 1), s, r(u[h]) ? u[h].length : 0), function(t) {
                    return s.decoder(t, n.decoder, d, "value");
                })), g && s.interpretNumericEntities && "iso-8859-1" === d && (g = o(String(g))), m.indexOf("[]=") > -1 && (g = r(g) ? [
                    g
                ] : g);
                var v = e.call(u, h);
                v && "combine" === s.duplicates ? u[h] = t.combine(u[h], g) : v && "last" !== s.duplicates || (u[h] = g);
            }
            return u;
        }(s, c) : s, p = c.plainObjects ? {
            __proto__: null
        } : {}, f = Object.keys(l), y = 0; y < f.length; ++y){
            var d = f[y], h = a(d, l[d], c, "string" == typeof s);
            p = t.merge(p, h, c);
        }
        return !0 === c.allowSparse ? p : t.compact(p);
    };
}
function Ne() {
    if (Gt) return Kt;
...
var Fe = Zt(Ne());
const ke = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:utils:url-helper");
class Le {
    static appendQueryParams(t, e) {
        if (ke("appending query params to %o: %o", t, e), !e) return ke("no query params provided, returning original URL: %o", t), t;
        const r = Le.stringifyQueryParams(e);
        return r ? URL.canParse(t) && new URL(t).search ? `${t}&${r}` : `${t}?${r}` : (ke("generated an empty query string, skipping..."), t);
    }
    static stringifyQueryParams(t) {
        return Fe.stringify(t, {
            addQueryPrefix: !1,
            allowEmptyArrays: !0
        });
    }
    static toReadablePath(t) {
...
class Ce {
    static format(t) {
        if (!(t instanceof Request)) throw new TypeError("Invalid input, expected a Request instance but found " + typeof t);
        return `${t.method} - ${Le.toReadablePath(t.url)}`;
    }
}
class Me {
    _options;
    _httpClient;
    constructor(t, e){
        this._options = t, this._httpClient = e;
    }
    get _resource() {
    ...
    get _pluginPrefix() {
        return void 0 !== this._options.plugin?.prefix ? this._options.plugin.prefix : this._pluginName ? this._pluginName : void 0;
    }
    get _rootPath() {
        if (this._path) return this._path;
        const t = this._pluginPrefix;
        return t ? `/${t}/${this._resource}` : `/${this._resource}`;
    }
}
const Be = {
    users: {
...
function qe(t) {
    ...
    const e = Object.values({
        ...Be,
        ...$e
    }).find((e)=>e.plugin.name === t);
    return e?.wrapsData ?? !0;
}
const He = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:ct:single");
class We extends Me {
    constructor(t, e){
        super(t, e), He('initialized a new "single" manager with %o', t);
    }
    shouldWrapDataBodyAttribute() {
        return qe(this._pluginName);
    }
    async find(t) {
        He("finding document for %o", this._resource);
        let e = this._rootPath;
        t && (e = Le.appendQueryParams(e, t));
        const r = await this._httpClient.get(e);
        return He("the %o document has been fetched", this._resource), r.json();
    }
    async update(t, e) {
        He("updating document for %o", this._resource);
        let r = this._rootPath;
        e && (r = Le.appendQueryParams(r, e));
        const n = await this._httpClient.put(r, JSON.stringify(this.shouldWrapDataBodyAttribute() ? {
            data: t
        } : t), {
            headers: {
                "Content-Type": "application/json"
        ...
        return He("the %o document has been updated", this._resource), n.json();
    }
    async delete(t) {
        He("deleting document for %o", this._resource);
        let e = this._rootPath;
        t && (e = Le.appendQueryParams(e, t)), await this._httpClient.delete(e), He("the %o document has been deleted", this._resource);
    }
}
const ze = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:ct:collection");
class Ve extends Me {
    constructor(t, e){
        super(t, e), ze('initialized a new "collection" manager with %o', t);
    }
    shouldWrapDataBodyAttribute() {
        return qe(this._pluginName);
    }
    async find(t) {
        ze("finding documents for %o", this._resource);
        let e = this._rootPath;
        t && (e = Le.appendQueryParams(e, t));
        const r = await this._httpClient.get(e), n = await r.json();
        return ze("found %o %o documents", Number(n?.data?.length), this._resource), n;
    }
    async findOne(t, e) {
        ze("finding a document for %o with id: %o", this._resource, t);
        let r = `${this._rootPath}/${t}`;
        e && (r = Le.appendQueryParams(r, e));
        const n = await this._httpClient.get(r);
        return ze("found the %o document with document id %o", this._resource, t), n.json();
    }
    async create(t, e) {
        ze("creating a document for %o", this._resource);
        let r = this._rootPath;
        e && (r = Le.appendQueryParams(r, e));
        const n = await this._httpClient.post(r, JSON.stringify(this.shouldWrapDataBodyAttribute() ? {
            data: t
        } : t), {
            headers: {
                "Content-Type": "application/json"
        ...
        return ze("created the %o document", this._resource), n.json();
    }
    async update(t, e, r) {
        ze("updating a document for %o with id: %o", this._resource, t);
        let n = `${this._rootPath}/${t}`;
        r && (n = Le.appendQueryParams(n, r));
        const o = await this._httpClient.put(n, JSON.stringify(this.shouldWrapDataBodyAttribute() ? {
            data: e
        } : e), {
            headers: {
                "Content-Type": "application/json"
        ...
        return ze("updated the %o document with id %o", this._resource, t), o.json();
    }
    async delete(t, e) {
        ze("deleting a document for %o with id: %o", this._resource, t);
        let r = `${this._rootPath}/${t}`;
        e && (r = Le.appendQueryParams(r, e)), await this._httpClient.delete(r), ze("deleted the %o document with id %o", this._resource, t);
    }
}
class Qe extends Ve {
}
class Ke extends l {
    ...
    fileId;
    constructor(t, e){
        super(e.response, e.request), this.fileId = t, this.message = `File with ID ${t} not found. The requested file may have been deleted or never existed.`;
    }
}
class Ge extends y {
    ...
    fileId;
    constructor(t, e){
        super(t.response, t.request), this.fileId = e, this.message = void 0 !== e ? `Access to file with ID ${e} is forbidden. You may not have sufficient permissions.` : "Access to files is forbidden. You may not have sufficient permissions.";
    }
}
class Je {
    static createMapper(t) {
        return (e)=>e instanceof l ? void 0 !== t ? new Ke(t, e) : e : e instanceof y ? new Ge(e, t) : e;
    }
}
const Ze = "/upload", Ye = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:files");
class Xe {
    _httpClient;
    constructor(t){
        this._httpClient = t, Ye("initialized files manager");
    }
    createFileHttpClient(t) {
        const e = this._httpClient.create(), r = Je.createMapper(t);
        return e.interceptors.response.use(void 0, (t)=>{
            if (t instanceof Error) throw r(t);
            throw t;
        }), e;
    }
    async find(t) {
        ...
        try {
            let e = `${Ze}/files`;
            t && (e = Le.appendQueryParams(e, t));
            const r = this.createFileHttpClient(), n = await r.get(e), o = await n.json();
            return Ye("found %o files", Number(o?.length)), o;
        } catch (t) {
            throw Ye("error finding files: %o", t), t;
        }
    }
    async findOne(t) {
        Ye("finding file with ID %o", t);
        const e = `${Ze}/files/${t}`, r = this.createFileHttpClient(t);
        try {
            const n = await r.get(e), o = await n.json();
            return Ye("found file with ID %o", t), o;
        } catch (e) {
            throw Ye("error finding file with ID %o: %o", t, e), e;
        }
    }
    async update(t, e) {
        Ye("updating file with ID %o with data %o", t, e);
        try {
            const r = `${Ze}?id=${t}`, n = this.createFileHttpClient(t), o = new FormData;
            o.append("fileInfo", JSON.stringify(e));
            const i = await n.post(r, o), a = await i.json();
            return Ye("successfully updated file with ID %o", t), a;
        } catch (e) {
            throw Ye("error updating file with ID %o: %o", t, e), e;
        }
    }
    async delete(t) {
        Ye("deleting file with ID %o", t);
        try {
            const e = `${Ze}/files/${t}`, r = this.createFileHttpClient(t), n = await r.delete(e), o = await n.json();
            return Ye("successfully deleted file with ID %o", t), o;
        } catch (e) {
            throw Ye("error deleting file with ID %o: %o", t, e), e;
        }
    }
    async upload(t, e) {
        Ye("uploading new file");
        try {
            const r = Ze, n = this.createFileHttpClient();
            if ("undefined" != typeof Buffer && Buffer.isBuffer(t)) {
                if (!e || !("filename" in e) || !("mimetype" in e)) throw new Error("Filename and mimetype are required when uploading Buffer data");
                return this.uploadBuffer(t, e, n, r);
            }
            return t instanceof Blob || console.warn("Could not determine type of file; attempting upload as Blob"), this.uploadBlob(t, e, n, r);
        } catch (t) {
            throw Ye("error uploading file: %o", t), t;
        }
    }
    async uploadBuffer(t, e, r, n) {
        const o = new FormData, i = new Blob([
            t
        ], {
            type: e.mimetype
        });
        o.append("files", i, e.filename), e.fileInfo && o.append("fileInfo", JSON.stringify(e.fileInfo));
        const a = await r.post(n, o), s = await a.json();
        return Ye("successfully uploaded buffer file"), s;
    }
    async uploadBlob(t, e, r, n) {
        const o = new FormData;
        o.append("files", t), e?.fileInfo && o.append("fileInfo", JSON.stringify(e.fileInfo));
        const i = await r.post(n, o), a = await i.json();
        return Ye("successfully uploaded blob file"), a;
    }
}
const tr = {
    trailingSlashes: !1,
...
class er {
    static format(t, e = tr) {
        return t = er.formatTrailingSlashes(t, e.trailingSlashes), t = er.formatLeadingSlashes(t, e.leadingSlashes);
    }
    static formatTrailingSlashes(t, e = tr.trailingSlashes) {
        return "single" === e ? er.ensureSingleTrailingSlash(t) : e ? t : er.removeTrailingSlashes(t);
    }
    static removeTrailingSlashes(t) {
        return t.replace(/\/+$/, "");
    }
    static ensureSingleTrailingSlash(t) {
        return `${this.removeTrailingSlashes(t)}/`;
    }
    static formatLeadingSlashes(t, e = tr.leadingSlashes) {
        return "single" === e ? er.ensureSingleLeadingSlash(t) : e ? t : er.removeLeadingSlashes(t);
    }
    static removeLeadingSlashes(t) {
        return t.replace(/^\/+/, "");
    }
    static ensureSingleLeadingSlash(t) {
        return `/${this.removeLeadingSlashes(t)}`;
    }
}
const rr = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:validators:url");
class nr {
    validate(t) {
        if ("string" != typeof t) throw rr("url must be a string, received: %o (%s)", t, typeof t), new s(t);
        if (!this.canParse(t)) throw rr("url could not be parsed: %o (%s)", t, typeof t), new s(t);
        rr("validated url successfully: %o", t);
    }
    canParse(t) {
        return URL.canParse(t);
    }
}
const or = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:validators:config");
class ir {
    _urlValidator;
    constructor(t = new nr){
        this._urlValidator = t;
    }
    validateConfig(t) {
        if (or("validating client config"), null == t || Array.isArray(t) || "object" != typeof t) throw or("provided client configuration is not a valid object: %o (%s)", t, typeof t), new o(new TypeError("The provided configuration is not a valid object."));
        this.validateBaseURL(t.baseURL), this.validateHeaders(t.headers), or("validated client config successfully");
    }
    validateBaseURL(t) {
        try {
            or("validating base url"), this._urlValidator.validate(t);
        } catch (e) {
            if (e instanceof a) throw or("failed to validate client config, invalid base url %o", t), new o(e);
            throw e;
        }
    }
    validateHeaders(t) {
        if (or("validating headers"), void 0 !== t) {
            if (null === t || "object" != typeof t || Array.isArray(t)) throw or("invalid headers type: %o (%s)", t, typeof t), new o(new TypeError("Headers must be a valid object."));
            for (const [e, r] of Object.entries(t))if ("string" != typeof r) throw or("invalid header value for key %s: %o (%s)", e, r, typeof r), new o(new TypeError("Header values must be strings."));
            or("headers validated successfully");
        }
    }
}
var ar;
!function(t) {
    t[t.OK = 200] = "OK", t[t.CREATED = 201] = "CREATED", t[t.NO_CONTENT = 204] = "NO_CONTENT", t[t.BAD_REQUEST = 400] = "BAD_REQUEST", t[t.UNAUTHORIZED = 401] = "UNAUTHORIZED", t[t.FORBIDDEN = 403] = "FORBIDDEN", t[t.NOT_FOUND = 404] = "NOT_FOUND", t[t.TIMEOUT = 408] = "TIMEOUT", t[t.INTERNAL_SERVER_ERROR = 500] = "INTERNAL_SERVER_ERROR";
}(ar || (ar = {}));
class sr {
    _handlers;
    constructor(t = []){
    ...
    async execute(t) {
        let e = t;
        for (const t of this._handlers)try {
            t.fulfilled && (e = await t.fulfilled(e));
        } catch (r) {
            if (!t.rejected) throw r;
            e = await t.rejected(r);
        }
        return e;
    }
    async reject(t) {
        let e = t;
        for (const t of this._handlers)t.rejected && (e = await t.rejected(e));
        return e;
    }
}
const ur = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:http");
class cr {
    interceptors;
    _baseURL;
    _timeout;
    _headers;
    _urlValidator;
    constructor(t, e = new nr){
        ur("initializing new client with base url: %o", t.baseURL), this._baseURL = er.format(t.baseURL, {
            trailingSlashes: !1
        }), this._timeout = t.timeout ?? 1e4, this._headers = t.headers ?? {}, this.interceptors = {
            request: new sr,
            response: new sr
        }, this._urlValidator = e, this._urlValidator.validate(this._baseURL);
    }
    get baseURL() {
    ...
    setBaseURL(t) {
        return this._urlValidator.validate(t), this._baseURL = er.format(t, {
            trailingSlashes: !1
        }), ur("setting new base url: %o", this._baseURL), this;
    }
    setTimeout(t) {
        if (ur("setting new timeout: %o", t), !Number.isSafeInteger(t)) throw new TypeError("Timeout must be a safe integer");
        return this._timeout = t, this;
    }
    async request(t, e) {
        const r = er.format(t, {
            leadingSlashes: "single"
        }), n = new URL(`${this.baseURL}${r}`), o = new Request(n, e);
        for (const [t, e] of Object.entries(this._headers))o.headers.append(t, e), ur("%o header set to %o for %o", t, e, Ce.format(o));
        const { request: i } = await this.interceptors.request.execute({
            request: o
        }), a = new AbortController, s = setTimeout(()=>a.abort(), this._timeout), u = new Request(i, {
            signal: a.signal
        });
        try {
            const t = await this.fetch(u);
            clearTimeout(s);
            const { response: e } = await this.interceptors.response.execute({
                request: u,
                response: t
            });
            return e;
        } catch (t) {
            throw clearTimeout(s), await this.interceptors.response.reject(t);
        }
    }
    fetch(t, e) {
        return globalThis.fetch(t, e);
    }
    async get(t, e) {
    ...
    create(t = {}, e = !0) {
        const r = new (Object.getPrototypeOf(this)).constructor({
            baseURL: t.baseURL ?? this._baseURL,
            timeout: t.timeout ?? this._timeout,
            headers: t.headers ?? this._headers
        }, this._urlValidator);
        if (!(r instanceof cr)) throw new Error("The created instance is not an instance of HttpClient");
        return e && (r.interceptors.request = this.interceptors.request.clone(), r.interceptors.response = this.interceptors.response.clone()), r;
    }
    static mapResponseToHTTPError(t, e) {
        switch(t.status){
            case ar.BAD_REQUEST:
                return new p(t, e);
            case ar.UNAUTHORIZED:
                return new c(t, e);
            case ar.FORBIDDEN:
                return new y(t, e);
            case ar.NOT_FOUND:
...
class lr {
    static ensurePreAuthentication(t, e) {
        return async ({ request: r })=>{
            const { strategy: n, isAuthenticated: o } = t;
            return n && !o && await t.authenticate(e), {
                request: r
            };
        };
    }
    static authenticateRequests(t) {
        return ({ request: e })=>(t.authenticateRequest(e), {
                request: e
            });
    }
    static notifyOnUnauthorizedResponse(t) {
        const e = ()=>t.handleUnauthorizedError();
        return [
            ({ request: t, response: r })=>(!r.ok && r.status === ar.UNAUTHORIZED && e(), {
                    request: t,
                    response: r
                }),
            (t)=>(t instanceof c && e(), t)
        ];
    }
}
class pr {
    static setDefaultHeaders() {
        ...
        return ({ request: e })=>{
            for (const [r, n] of t.entries()){
                if (!e.headers.has(r)) {
                    if ("Content-Type" === r && e.body instanceof FormData) continue;
                    e.headers.set(r, n);
                }
            }
            return {
    ...
    static transformErrors() {
        return ({ request: t, response: e })=>{
            ...
            throw cr.mapResponseToHTTPError(e, t);
        };
    }
}
const fr = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$debug$2f$src$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"])("strapi:core");
class yr {
    _config;
    _validator;
    _authManager;
    _httpClient;
    files;
    constructor(t, e = new ir, r = new A, n){
        this._config = t, this._validator = e, this._authManager = r, fr("started the initialization process"), this.preflightValidation(), fr("user config passed the preflight validation"), this._httpClient = n ? n({
            baseURL: t.baseURL,
            headers: t.headers
        }) : new cr({
            baseURL: t.baseURL,
            headers: t.headers
        }), this.files = new Xe(this._httpClient), this.init(), fr("finished the client initialization process");
    }
    preflightValidation() {
        try {
            fr("validating the configuration"), this._validator.validateConfig(this._config);
        } catch (t) {
            throw new i(t);
        }
    }
    init() {
        fr("init modules"), this.initHttp(), this.initAuth();
    }
    initHttp() {
        fr("init http module"), this._httpClient.interceptors.request.use(pr.setDefaultHeaders()), this._httpClient.interceptors.response.use(pr.transformErrors());
    }
    initAuth() {
        if (fr("init auth module"), this.auth) {
            const { strategy: t, options: e } = this.auth;
            fr("setting up the auth strategy using %o", t);
            try {
                this._authManager.setStrategy(t, e);
            } catch (t) {
                throw new i(t, `Failed to initialize the client auth manager: ${t instanceof n ? t.cause : t}`);
            }
        }
        this._httpClient.interceptors.request.use(lr.ensurePreAuthentication(this._authManager, this._httpClient)).use(lr.authenticateRequests(this._authManager)), this._httpClient.interceptors.response.use(...lr.notifyOnUnauthorizedResponse(this._authManager));
    }
    get auth() {
    ...
    fetch(t, e) {
        return this._httpClient.request(t, e);
    }
    collection(t, e = {}) {
        const { path: r, plugin: n } = e, o = function(t) {
            return Be[t];
        }(t), i = n ?? o?.plugin ?? void 0;
        if ("users" === t && "users-permissions" === i?.name) {
            return new Qe({
                resource: t,
                path: r,
                plugin: i
    ...
    single(t, e = {}) {
        ...
        return new We({
            resource: t,
            path: r,
            plugin: n ?? o?.plugin ?? void 0
        }, this._httpClient);
    }
}
const dr = (t)=>{
    const { baseURL: e, auth: r, headers: n } = t, o = {
        baseURL: e,
        headers: n
    };
    return void 0 !== r && (o.auth = {
        strategy: g.identifier,
        options: {
            token: r
        }
    }), new yr(o);
...
//# sourceMappingURL=node_modules_e1fec6ab._.js.map