    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-[60] transition-all duration-500 ${
          isScrolled || isMenuOpen ? 'bg-white/[0.90] backdrop-blur-md shadow-sm' : 'bg-gradient-to-b from-black/20 to-transparent'
        }`}
      >
        <div className="container mx-auto px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <RouterLink
              to="home"
              className={`smooth duration={500} relative z-10 cursor-pointer transition-all duration-500 ${
                isScrolled || isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'
              }`}
            >
              <div className="flex flex-col items-center">
                <span className="text-xl font-semibold text-[#1D1D1E] relative">
                  {logoText}
                  <span className="absolute -bottom-1 left-0 w-full h-[1px] bg-[#1D1D1E]" />
                </span>
...
<nav className="hidden lg:flex items-center space-x-8">
    ...
    dynamicMenuItems.map((item, idx) =>
      item.submenu && item.submenu.length > 0 ? (
        ...
        >
          <button
            className={`flex items-center gap-1 cursor-pointer transition-colors ${
              isScrolled ? 'text-[#38363C]' : 'text-white'
            } hover:text-[#B4943E]`}
          >
            {item.label} <ChevronDownIcon size={16} />
          </button>
          <div
            className={`absolute top-full left-0 mt-2 w-48 bg-white rounded-lg shadow-lg transition-all duration-200 ${
        ...
        <Link
          ...
          smooth={true}
          duration={500}
          className={`cursor-pointer transition-colors hover:text-[#B4943E] ${
            isScrolled ? 'text-[#38363C]' : 'text-white'
          }`}
        >
          {item.label}
  ...
  >
    Időpontfoglalás
            ...
            <button
              className={`lg:hidden z-[70] w-6 h-6 relative focus:outline-none ${
                isScrolled || isMenuOpen ? 'text-[#38363C]' : 'text-white'
              }`}
              onClick={() => setIsMenuOpen(!isMenuOpen)}
            >
              <span
                className={`absolute w-full h-0.5 transition-all duration-300 transform ${
                  isMenuOpen
